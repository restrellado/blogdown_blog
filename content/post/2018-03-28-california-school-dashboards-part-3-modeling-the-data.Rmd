...
we visualized the dashboard data [in part of two of this series](http://ryanestrellado.netlify.com/post/california-school-dashboards-part-2-visualizing-the-data/). For review, recall that the distance from three score for English languae arts is the
...
Now it's a little clearer that there might be some differences in the way the model fits for the ELO subgroup. We won't go into the differences in the way the California Department of Education distinguishes ELs from ELOs, but it's important to note that in a real analysis it would be critical to understand how these subgroups are defined. You can read more about the [definition of EL and ELO in this CDE document](https://www.cde.ca.gov/ta/ac/cm/documents/dashboardguidespring17.pdf). 

# Practical Next Steps 

Now that we have this information, what do we do with it? Here are some practical next steps 
for building on the analysis turning it into meaningful action: 

Build consensus on conclusions by examining other sources of data. Questions to ask might be: 
 
 - Do these findings resonate with the accounts of teachers? 
 - Are there are quantitative and anecdotal datasets, like local measures or weekly quiz scores that reinforce or challenge these findings? 
 
Look at the underlying instrutional processes that might logically drive these findings. Ask 
questions like: 
